...
const getAllTrackSchema = z.object({
   query: z.object({
      ...
      categoryStatus: optionalEnumString(
...
// 3.1. GET ALL TRACK
const getAllPublishedTrackSchema = z.object({
   query: z.object({
      page: optionalNumber("Page"),
      limit: optionalNumber("Limit"),
      searchTerm: optionalString("Search term"),
...
// 6. Mark as Published
const markTrackAsPublishedSchema = z.object({
   params: z.object({
      id: requiredMongooseId("Track ID"),
   }),
});
...
// 7. Mark as Archived
const markTrackAsArchivedSchema = z.object({
   params: z.object({
      id: requiredMongooseId("Track ID"),
   }),
});
...
export const trackValidations = {
   ...
   updateTrackSchema,
   getAllTrackSchema,
   getAllPublishedTrackSchema,
   getTrackByIdSchema,
   deleteTrackByIdSchema,
   markTrackAsPublishedSchema,
   markTrackAsArchivedSchema,
};
...
export type TGetAllActiveTrackQueryParamsType = z.infer<
   typeof getAllPublishedTrackSchema.shape.query
>;
